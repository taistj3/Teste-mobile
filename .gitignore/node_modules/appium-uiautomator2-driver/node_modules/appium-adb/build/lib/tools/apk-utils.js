...
apkUtilsMethods.cacheApk = async function cacheApk(apkPath, options = {}) {
    ...
    const remotePath = path_1.default.posix.join(REMOTE_CACHE_ROOT, `${appHash}.apk`);
    ...
    try {
        ...
        if (!lodash_1.default.isString(lsOutput) || (lsOutput.includes(errorMarker) && !lsOutput.includes(REMOTE_CACHE_ROOT))) {
    ...
    const entriesToCleanup = remoteCachedFiles
        .map((x) => path_1.default.posix.join(REMOTE_CACHE_ROOT, x))
...
apkUtilsMethods.install = async function install(appPath, options = {}) {
    ...
    if (shouldCacheApp) {
        const clearCache = async () => {
            logger_js_1.default.info(`Clearing the cache at '${REMOTE_CACHE_ROOT}'`);
...
apkUtilsMethods.getApplicationInstallState = async function getApplicationInstallState(appPath, pkg = null) {
    ...
    const pkgVersionName = semver_1.default.valid(semver_1.default.coerce(pkgVersionNameStr));
    if (!apkInfo) {
        apkInfo = await this.getApkInfo(appPath);
    }
    // @ts-ignore We validate the valus below
    const { versionCode: apkVersionCode, versionName: apkVersionNameStr } = apkInfo;
    const apkVersionName = semver_1.default.valid(semver_1.default.coerce(apkVersionNameStr));
    ...
    if (lodash_1.default.isInteger(apkVersionCode) && lodash_1.default.isInteger(pkgVersionCode)) {
        ...
        if (pkgVersionCode === apkVersionCode) {
            if (lodash_1.default.isString(apkVersionName) && lodash_1.default.isString(pkgVersionName) && semver_1.default.satisfies(pkgVersionName, `>=${apkVersionName}`)) {
                ...
                return semver_1.default.satisfies(pkgVersionName, `>${apkVersionName}`)
    ...
    else if (lodash_1.default.isString(apkVersionName) && lodash_1.default.isString(pkgVersionName) && semver_1.default.satisfies(pkgVersionName, `>=${apkVersionName}`)) {
        logger_js_1.default.debug(`The version name of the installed '${pkg}' is greater or equal to the application version name ('${pkgVersionName}' >= '${apkVersionName}')`);
        return semver_1.default.satisfies(pkgVersionName, `>${apkVersionName}`)
            ? this.APP_INSTALL_STATE.NEWER_VERSION_INSTALLED
            : this.APP_INSTALL_STATE.SAME_VERSION_INSTALLED;
...
apkUtilsMethods.extractStringsFromApk = async function extractStringsFromApk(appPath, language = null, outRoot = null) {
    ...
    try {
        ...
        configMarker = await (0, helpers_js_1.formatConfigMarker)(async () => {
            const { stdout } = await (0, teen_process_1.exec)(( /** @type {import('@appium/types').StringRecord} */(this.binaries)).aapt, [
        ...
        configMarker = await (0, helpers_js_1.formatConfigMarker)(async () => {
            const { stdout } = await (0, teen_process_1.exec)(( /** @type {import('@appium/types').StringRecord} */(this.binaries)).aapt2, [
                'd', 'configurations', appPath,
            ]);
            return lodash_1.default.uniq(stdout.split(os_1.default.EOL));
        }, language, '');
...
apkUtilsMethods.ensureCurrentLocale = async function ensureCurrentLocale(language, country, script) {
    const hasLanguage = lodash_1.default.isString(language);
    const hasCountry = lodash_1.default.isString(country);
    if (!hasLanguage && !hasCountry) {
        logger_js_1.default.warn('ensureCurrentLocale requires language or country');
        return false;
    }
    ...
    return /** @type {boolean} */ (await (0, asyncbox_1.retryInterval)(5, 1000, async () => {
        if (apiLevel < 23) {
            logger_js_1.default.debug(`Requested locale: ${lcLanguage}-${lcCountry}`);
            let actualLanguage;
            if (hasLanguage) {
                actualLanguage = (await this.getDeviceLanguage()).toLowerCase();
                logger_js_1.default.debug(`Actual language: ${actualLanguage}`);
            ...
            if (hasCountry) {
                ...
                logger_js_1.default.debug(`Actual country: ${actualCountry}`);
        ...
        logger_js_1.default.debug(`Requested locale: ${expectedLocale}. Actual locale: '${actualLocale}'`);
        const languagePattern = `^${lodash_1.default.escapeRegExp(lcLanguage)}-${script ? (lodash_1.default.escapeRegExp(script) + '-') : ''}`;
        const checkLocalePattern = (/** @type {string} */ p) => new RegExp(p, 'i').test(actualLocale);
        if (hasLanguage && !hasCountry) {
            return checkLocalePattern(languagePattern);
        }
        const countryPattern = `${script ? ('-' + lodash_1.default.escapeRegExp(script)) : ''}-${lodash_1.default.escapeRegExp(lcCountry)}$`;
        if (!hasLanguage && hasCountry) {
...
apkUtilsMethods.activateApp = async function activateApp(appId) {
    ...
    if (apiLevel < 24) {
        ...
        try {
            ...
            logger_js_1.default.errorAndThrow(`Cannot activate '${appId}'. Original error: ${e.message}`);
        }
        if (output.includes('monkey aborted')) {
            logger_js_1.default.errorAndThrow(`Cannot activate '${appId}'. Are you sure it is installed?`);
        }
        return;
    }
